...
export class content_dashboard extends Component {
    ...
    confirmDestination = async () =>{
        ...
        if(receipientInfo !== '' && amount !== '' && network !== '' && amount !== 0 ){
            ...
            console.log('all')
            let response = await axios.post('http://localhost:4000/api/users/confirmDestination', 
    ...
    creditAccount = async () => {
        const response  = await axios.post('http://localhost:4000/api/users/creditAccount',
