...
export class content_dashboard extends Component {
    ...
    selectedCard = async () => {
        try {
            const response  = await axios.get('http://10.0.2.15:3000/api/users/cards/selectedCard', {headers: 
    ...
    confirmDestination = async () =>{
        ...
        if(receipientInfo !== '' && amount !== '' && network !== '' && amount !== 0 ){
            ...
            console.log('all')
            let response = await axios.post('http://10.0.2.15:3000/api/users/confirmDestination', 
    ...
    creditAccount = async () => {
        const response  = await axios.post('http://10.0.2.15:3000/api/users/creditAccount',
